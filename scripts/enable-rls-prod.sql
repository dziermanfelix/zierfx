-- Enable RLS on all tables (handles existing policies gracefully)
...
DROP POLICY IF EXISTS "Allow all operations on Artist" ON "Artist";
DROP POLICY IF EXISTS "Allow all operations on Album" ON "Album";
DROP POLICY IF EXISTS "Allow all operations on Track" ON "Track";
DROP POLICY IF EXISTS "Allow all operations on User" ON "User";
DROP POLICY IF EXISTS "Allow all operations on Venue" ON "Venue";
DROP POLICY IF EXISTS "Allow all operations on Show" ON "Show";
